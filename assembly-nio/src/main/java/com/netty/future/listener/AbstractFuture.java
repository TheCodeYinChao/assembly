...
public abstract class AbstractFuture<V> implements IFuture<V> {
    ...
    protected Collection<IFutureListener> listeners = new CopyOnWriteArrayList<IFutureListener>();
    ...
    @Override
    public IFuture<V> await() throws InterruptedException, ExecutionException {
        return realAwait(true);
    }

    private IFuture<V> realAwait(boolean interruptable) throws InterruptedException {
    ...
    @Override
    public IFuture<V> awaitUninterruptibly() throws Exception {
        try {
    ...
    @Override
    public IFuture<V> addListener(IFutureListener listener) {
        if (listener == null) {
    ...
    @Override
    public IFuture<V> removeListener(IFuture<V> listener) {
        if (listener == null) {
            throw new NullPointerException("listener");
        }
